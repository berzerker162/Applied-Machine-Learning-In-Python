...
def answer_one():
    ...
    ans = df[df['Class']==1]['Class'].sum() / df['Class'].count()
...
def answer_six():    
    ...
    grid_logreg = GridSearchCV(estimator = logreg, param_grid = params,
                               scoring = 'recall', cv=3)
    
    grid_logreg.fit(X_train, y_train)  
    
    return grid_logreg.cv_results_['mean_test_score'].reshape(5,2)
